...
export function WhatsAppFloat() {
  return (
    <a
      ...
      className="group fixed bottom-5 right-5 z-40 grid h-14 w-14 place-items-center rounded-full bg-[#25D366] text-white shadow-[var(--shadow-elegant)] transition-transform duration-300 hover:scale-110 sm:bottom-6 sm:right-6"
    >
      {/* Pulsing halo */}
      <span
        aria-hidden
        className="pointer-events-none absolute inset-0 rounded-full bg-[#25D366] opacity-70 animate-ping"
      />
      {/* Secondary softer ring */}
      <span
        aria-hidden
        className="pointer-events-none absolute -inset-1 rounded-full border border-[#25D366]/40"
      />
      <svg
        viewBox="0 0 32 32"
        className="relative h-7 w-7 transition-transform duration-500 group-hover:rotate-[12deg]"
        fill="currentColor"
        aria-hidden
      >
